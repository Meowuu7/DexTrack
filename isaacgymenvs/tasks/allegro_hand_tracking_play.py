...
class AllegroHandTrackingPlay(BaseTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        # debug viz #
        self.debug_viz = self.cfg["env"]["enableDebugVis"] # enable the debug viz #
        ...
        self.masked_mimic_training = False
        self.train_student_model = False
        ...
        if self.viewer != None:
            ...
            # cam_pos = gymapi.Vec3(10.0, 7.9, 1.0)
            # cam_target = gymapi.Vec3(9, 7.9, 0.5)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        goal_start_pose.p.z -= 9.0 # goal start pose # # goal pose #
    ...
    def pre_physics_step(self, actions):
        ...
        # 
        cur_replay_progress_buf= torch.clamp(self.progress_buf, min=torch.zeros_like(self.progress_buf), max=torch.zeros_like(self.progress_buf) + self.optimized_hand_qs.size(0) - 1)
        cur_hand_replay_qs = batched_index_select(self.optimized_hand_qs, cur_replay_progress_buf.unsqueeze(1), dim=0)
        cur_hand_replay_qs = cur_hand_replay_qs.squeeze(1) # nn_envs x nn_hand_qs #
        # print(f"hand_qs: {cur_hand_replay_qs.size()}")
        ...
        cur_obj_replay_pos = batched_index_select(self.optimized_obj_pose[..., :3], cur_replay_progress_buf.unsqueeze(-1), dim=0)
        cur_obj_replay_pos = cur_obj_replay_pos.squeeze(1)

        # nn_envs x 4 --- object orientations #
        cur_obj_replay_ornt = batched_index_select(self.optimized_obj_pose[..., 3:], cur_replay_progress_buf.unsqueeze(-1), dim=0)
