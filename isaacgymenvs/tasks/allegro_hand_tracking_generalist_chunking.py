...
class AllegroHandTrackingGeneralist(BaseTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        # # TODO: ts settings for vision baed pipelines # #
        ...
        ### whether to add the physical parameters in the observations ### # add physical #
        self.add_physical_params_in_obs = self.cfg['env'].get('add_physical_params_in_obs', False)
        # add four dimensions -- #
        
        # action chunking frames #
        self.action_chunking_frames = self.cfg['env'].get('action_chunking_frames', 1)
        # step --- so we need to change the chunking logitcs? or only the model with the chunking frames #
        
        self.multi_inst_chunking = self.cfg['env'].get('multi_inst_chunking', False)
        
        self.bc_style_training = self.cfg['env'].get('bc_style_training', False)
        self.bc_relative_targets = self.cfg['env'].get('bc_relative_targets', False)
        
        self.load_chunking_experiences_v2 = self.cfg['env'].get('load_chunking_experiences_v2', False)
        ...
        self.add_obj_features = self.cfg['env'].get('add_obj_features', False)
        ...
        self.switch_to_trans_model_frame_after = self.cfg['env'].get('switch_to_trans_model_frame_after', False)
        self.switch_to_trans_model_ckpt_fn = self.cfg['env'].get('switch_to_trans_model_ckpt_fn', False)
        
        self.history_chunking_obs_version = self.cfg['env'].get('history_chunking_obs_version', 'v1')
        ...
        def get_all_grab_tags():
            ...
            for cur_fn in tot_fns:
                ...
                if self.grab_train_test_setting: # judge whether the modle is in the training split mode 
        ...
        if len(self.test_inst_tag) > 0 and len(self.test_optimized_res) > 0 and os.path.exists(self.test_optimized_res):
            ...
            # with obj code file name # # # file name # # file name # 
        ...
        if self.bc_style_training:
            ...
            if self.load_chunking_experiences_v2:
                if self.history_chunking_obs_version == 'v1':
                    obs_dim_bc_chunking = (7 + 16 + 7) * self.history_length + (16 + 7) * self.history_length + (16 + 7 + 7) * self.action_chunking_frames
                elif self.history_chunking_obs_version == 'v2':
                    obs_dim_bc_chunking = (7 + 16 + 7) * self.history_length 
                    # + (16 + 7) * self.history_length + (16 + 7 + 7) * self.action_chunking_frames
                else:
                    raise ValueError(f"Unrecognized history_chunking_obs_version: {self.history_chunking_obs_version}")
        ...
        if self.action_chunking_frames:
            self.cfg['env']['numActions'] = self.nn_hand_dof * self.action_chunking_frames
        
        
        # actions --- a window of actions #
        # history --- add a history of hand qpos #
        ...
        self.history_rew_buf = torch.zeros((self.history_buf_length, self.num_envs), device=self.device)  # 
        ...
        if self.w_franka:
            # actuated_dof_indices_fly #  # prev_targets_fly, cur_targets_fly
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # object code: sem/pure_obj_name # # create envs #
        ...
        for i in range(self.num_envs):
            ...
            if self.disable_hand_obj_contact:
    ...
    def compute_full_state_action_chunking(self):
        # hand_dof_pos_state = self.shadow_hand_dof_pos # nn_envs x nn_hand_dofs #
        
        tot_history_hand_dof_pos = self.tot_history_hand_dof_pos
        tot_history_object_pose = self.tot_history_object_pose
        tot_history_right_hand_actions = self.tot_history_right_hand_actions
        
        # future_progress_bufs = torch.arange(1, self.action_chunking_frames + 1, step=1).long().to(self.rl_device)
        future_progress_bufs = torch.arange(0, self.action_chunking_frames, step=1).long().to(self.rl_device)
        future_progress_bufs = self.progress_buf.unsqueeze(1) + future_progress_bufs.unsqueeze(0) # nn_envs x nn_chunks
        envs_maxx_traj_lengths = batched_index_select(self.maxx_episode_length_per_traj, self.env_inst_idxes, dim=0)
        future_progress_bufs = torch.clamp(
            future_progress_bufs, min=torch.zeros_like(future_progress_bufs), max=envs_maxx_traj_lengths.unsqueeze(1).contiguous().repeat(1, self.action_chunking_frames)
        ) # nn_envs x nn_chunks
        ...
        tot_goal_hand_qs_th = self.tot_kine_qs_w_arm
        
        
        envs_goal_hand_qs = batched_index_select(tot_goal_hand_qs_th, self.env_inst_idxes, dim=0) ## nn_envs x maximum_episode_length x len(hand_qs)
        
        goal_obj_trans_th = batched_index_select(envs_goal_obj_trans_th, future_progress_bufs, dim=1)
        goal_obj_rot_th = batched_index_select(envs_goal_obj_ornt_th, future_progress_bufs, dim=1)
        goal_hand_qpos_th = batched_index_select(envs_goal_hand_qs, future_progress_bufs, dim=1)
        
        goal_vecs = torch.cat(
            [ goal_hand_qpos_th, goal_obj_trans_th, goal_obj_rot_th], dim=-1
        )
        
        if self.history_chunking_obs_version == 'v1':
            history_vecs = torch.cat(
                [ tot_history_hand_dof_pos, tot_history_object_pose, tot_history_right_hand_actions ], dim=-1
            )
        else:
            history_vecs = torch.cat(
                [ tot_history_hand_dof_pos, tot_history_object_pose ], dim=-1
            )
        
        history_vecs = history_vecs.contiguous().view(history_vecs.size(0), -1).contiguous()
        goal_vecs = goal_vecs.contiguous().view(goal_vecs.size(0), -1).contiguous()
        
        if self.history_chunking_obs_version == 'v1':
            obs_vecs = torch.cat(
                [ history_vecs, goal_vecs ], dim=-1
            )
        else:
            obs_vecs = history_vecs
        
        print(f"obs_vecs: {obs_vecs.size()}, obs_buf: {self.obs_buf.size()}")
        
        self.obs_buf[:, : obs_vecs.size(-1)] = obs_vecs[:, :]
    ...
    def compute_observations(self):
        ...
        ### TODO: we first try to not to use the forecasted hand qpos ###
        ...
        if self.test:
            ...
            self.actions_np = self.target_qpos_np
        ...
        if (not self.to_real) and self.bc_style_training:
    ...
    def compute_full_state(self, asymm_obs=False): #
        ...
        if not self.use_history_obs:
            ...
            self.cur_dof_vel[:, : ] = self.shadow_hand_dof_vel # g
            ...
            # add the history shadow hand dof #
            canon_shadow_hand_dof = canon_shadow_hand_dof.contiguous().view(canon_shadow_hand_dof.size(0), -1).contiguous() 
            self.obs_buf[:, 0:self.num_shadow_hand_dofs * self.history_length] = canon_shadow_hand_dof
            
            
            # add the history shadow hand dof velocities #
            if self.wo_vel_obs: # previous hand poses and dofs and the preiv
                self.obs_buf[:, self.num_shadow_hand_dofs * self.history_length : 2 * self.num_shadow_hand_dofs * self.history_length] = 0.0
            else:
                self.obs_buf[:,self.num_shadow_hand_dofs * self.history_length :2 * self.num_shadow_hand_dofs * self.history_length ] = self.vel_obs_scale * self.tot_history_hand_dof_vel.contiguous().view(self.tot_history_hand_dof_vel.size(0), -1).contiguous() # get the hand dof velocities #
            ...
            ## TODO: if without the fingertip velocities or the orientations, we should replace fingertip states with the corresponding positions or positions with orientations ##
            # if self.wo_fingertip_rot_vel:
            #     canon_fingertip_pose[..., 3:] = canon_fingertip_pose[..., 3:] * 0.0 # zero out the fingertip rotation velocities #
            # elif self.wo_fingertip_vel:
            #     canon_fingertip_pose = torch.cat(
    ...
    # compute the full teacher state #
    ...
    def reset(self, env_ids=None, goal_env_ids=None): 
        ...
        # 
        if self.to_real and self.closed_loop_to_real and self.bc_style_training:
            init_states = self.set_initial_real_state()
            obs_dict = {}
            if self.add_obj_features:
                init_states = torch.cat(
                    [ init_states, self.obj_feature], dim=-1
                )
            obs_dict['obs'] = init_states 
            ...
            # compute observations in the sim #
            # if not self.bc_style_training:
            ...
            if self.masked_mimic_training:
                ...
                return self.obs_buf #  torch.clamp(self.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device)
    ...
    def pre_physics_step(self, actions):
        ...
        # add the specific joint targets to hand dofs #
        # add specifiic joint targets to hand dofs? #
    ...
    def post_physics_step(self):
        ...
        # self.rew_buf # 
        ## add to the histroy rew buf #  # (nn_envs, ) # (self.nn_envs, history_buf_len)
        cur_progress_buf_expanded = (self.progress_buf - 1).unsqueeze(-1).contiguous().repeat(1, self.history_buf_length) # (nn_envs x nn_len_history) 
        # (nn_envs, )
        expanded_cur_rew_buf = self.rew_buf.unsqueeze(-1).contiguous().repeat(1, self.history_buf_length) # (nn_envs x nn_len_history) # nn_envs x history buf lenght 
        # expanded cur buf --- set 
        epanded_aragned_progress_idxes = torch.arange(self.history_buf_length, device=self.rl_device).unsqueeze(0).contiguous().repeat(self.num_envs, 1) # (nn_envs x nn_len_history) #
        self.history_rew_buf = torch.where( # get the expanded cur rew buf #
            cur_progress_buf_expanded == epanded_aragned_progress_idxes, expanded_cur_rew_buf, self.history_rew_buf.contiguous().transpose(1, 0).contiguous()
    ...
    def step(self, actions: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor, Dict[str, Any]]:
        ...
        if self.to_real and self.closed_loop_to_real and self.bc_style_training:
            if self.load_chunking_experiences_v2:
                obs, rew, reset, extras = self.step_chunking_v2(actions)
            else:
                obs, rew, reset, extras = self.step_chunking(actions)
            return obs, rew, reset, extras 
            # yield self.step_chunking(actions)
        ...
        chunks_action_tensor = action_tensor.contiguous().view(self.num_envs, self.action_chunking_frames, -1)
        
        
        for i_chunk in range(self.action_chunking_frames):
        # for i_chunk in range(2):
            # # chunking # # # chunking #
            cur_action_tensor = chunks_action_tensor[:, i_chunk ]
            # apply the actions onto the model #
            # self.pre_physics_step(cur_action_tensor)
            
            self.cur_targets[:, self.actuated_dof_indices] = cur_action_tensor[:, :]
            self.prev_targets[:, self.actuated_dof_indices] = self.cur_targets[:, self.actuated_dof_indices]
        ...
        if self.train_student_model:
            self.obs_dict['full_obs'] = torch.clamp(self.full_obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device)    
        ...
        tot_chunking_progress_bufs = []
        for i_chunk in range(1, self.action_chunking_frames + 1, 1):
            chunk_progress_buf = self.progress_buf - i_chunk
            chunk_progress_buf = torch.clamp(chunk_progress_buf, min=0, max=self.history_buf_length - 1) # 
            tot_chunking_progress_bufs.append(chunk_progress_buf)
        # tot_rpgoress_bus # 
        tot_chunking_progress_bufs = torch.stack(tot_chunking_progress_bufs, dim=-1) # (nn_envs x nn_chunk_frames)
        # transposed_history_rew_buf: nn_envs x nn_chunk_frames #
        # nn_envs x nn_chunk_frames #
        tot_chunking_rews = batched_index_select(self.history_rew_buf.contiguous().transpose(1, 0).contiguous(), tot_chunking_progress_bufs, dim=1) # (nn_envs x nn_chunk_frames) 
        # tot_avg_chunking_rews = torch.mean(tot_chunking_rews, dim=-1) # (nn_envs, ) 
        tot_avg_chunking_rews = torch.sum(tot_chunking_rews, dim=-1) # (nn_envs, ) --- chunking rews #
        
        return self.obs_dict, tot_avg_chunking_rews.to(self.rl_device), self.reset_buf.to(self.rl_device), self.extras
        # accrmulated rew or mean of the rewards? #
