...
class ShadowHandTracking(BaseTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        self.nn_hand_dof = self.cfg["env"]["nn_hand_dof"]
    ...
    def compute_reward(self, actions, id=-1):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.current_successes[:], self.consecutive_successes[:] = compute_hand_reward_tracking( 
            ...
            self.right_hand_th_pos, self.right_hand_lf_pos,
...
def compute_hand_reward_tracking(
        ...
        right_hand_pos, right_hand_ff_pos, right_hand_mf_pos, right_hand_rf_pos, right_hand_th_pos, right_hand_lf_pos,
        dist_reward_scale: float, rot_reward_scale: float, rot_eps: float,
        actions, action_penalty_scale: float,
        success_tolerance: float, reach_goal_bonus: float, fall_dist: float,
    ...
    right_hand_finger_dist = (torch.norm(object_handle_pos - right_hand_ff_pos, p=2, dim=-1) + torch.norm(
        object_handle_pos - right_hand_mf_pos, p=2, dim=-1)+ torch.norm(object_handle_pos - right_hand_rf_pos, p=2, dim=-1)  + torch.norm(object_handle_pos - right_hand_th_pos, p=2, dim=-1)) + torch.norm(
                object_handle_pos - right_hand_lf_pos, p=2, dim=-1) 
    ...
    if goal_cond: ## 
        ...
        reward = -0.5 * delta_value + (-0.5) * right_hand_finger_dist  + goal_hand_rew + bonus + hand_up 
