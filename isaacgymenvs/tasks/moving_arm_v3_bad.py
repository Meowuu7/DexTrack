...
class MovingArm(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_id, object_code in enumerate(self.object_code_list):
            ...
            goal_asset = self.gym.create_sphere(self.sim, 0.01, object_asset_options)
            # >>>>>>>> mx_debug <<<<<<<<
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        # self.root_state_tensor[self.goal_object_indices[env_ids], 0:1] = self.root_state_tensor[self.fullbody_indices[env_ids], 0:1] + grasp_front_dist
    ...
    def pre_physics_step(self, actions):
        ...
        all_fullbody_and_goal_indices = torch.unique(torch.cat([self.fullbody_indices,self.goal_object_indices]).to(torch.int32)) 
        ...
        self.gym.set_actor_root_state_tensor_indexed(self.sim, gymtorch.unwrap_tensor(arm_root_state),
                                                         gymtorch.unwrap_tensor(all_fullbody_and_goal_indices),
                                                         len(all_fullbody_and_goal_indices))
        
        # gym.set_dof_target_velocity(env2, cart_dof_handle2, -1.0)
        debug = 10
        # self.gym.get_dof_velocity(self.envs[0], 0)
        # self.gym.get_dof_position(self.envs[0], 0)
        
        # self.gym.set_dof_target_velocity(env2, cart_dof_handle2, 1.0)
        # slider_dof_handle = self.gym.find_actor_dof_handle(self.envs[0], 0, 'slider')

        # for ii in range(len(self.envs)):
        #     self.gym.set_dof_target_position(self.envs[ii], 0, 0.2)
        
            # self.gym.apply_dof_effort(self.envs[ii], 0, 1.0)
        # gym.set_dof_target_velocity(env2, cart_dof_handle2, 1.0)
        # get_dof_position
