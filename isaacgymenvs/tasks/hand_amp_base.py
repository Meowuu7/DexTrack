...
class HandAMPBase(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        self.num_agents = 1
        # TODO 这里决定num actions
        # self.cfg["env"]["numActions"] = 24
        # sh_dof=22, 6 for force and torque
        self.cfg["env"]["numActions"] = 22+6
        ...
        self._local_root_obs = self.cfg["env"].get("localRootObs", False)
        self._root_height_obs = self.cfg["env"].get("rootHeightObs", False)
    ...
    def _setup_character_props(self, key_bodies):
        ...
        self._num_actions = self.cfg["env"]["numActions"]
        self._num_actions = 22
        self._num_obs = self.cfg["env"]["numObservations"]
    ...
    def _build_key_body_ids_tensor(self, key_body_names):
        env_ptr = self.envs[0]
        actor_handle = self.shadow_hand_actor_handles[0]
        body_ids = []
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        shadow_hand_asset_file = "mjcf/open_ai_assets/hand/hand_nowrj.xml"
        # shadow_hand_asset_file = "mjcf/open_ai_assets/hand/fullbody.xml"
        # shadow_hand_asset_file = "shadow_hand.xml"
        ...
        self.shadow_hand_actor_handles = []
        self.sensors = []
        ...
        rx, ry = 0.0, 0.0
        rz = 1.0 
        # shadow_hand_start_pose.p = gymapi.Vec3(0.3, 0.3, 1.2)  # gymapi.Vec3(0.1, 0.1, 0.65)
        shadow_hand_start_pose.p = gymapi.Vec3(rx, ry, table_z+0.5)  # gymapi.Vec3(0.1, 0.1, 0.65)
        shadow_hand_start_pose.r = gymapi.Quat().from_euler_zyx(1.57, 0, 0)  # gymapi.Quat().from_euler_zyx(0, -1.57, 0)

        object_start_pose = gymapi.Transform()
        object_start_pose.p = gymapi.Vec3(0.0, 0.0, table_z + 0.1)  # gymapi.Vec3(0.0, 0.0, 0.72)
        ...
        for i in range(self.num_envs):
            ...
            self.shadow_hand_actor_handles.append(shadow_hand_actor)
    ...
    def pre_physics_step(self, actions):
        ...
        self.cur_targets = scale(self.actions[:, 6:],self.shadow_hand_dof_lower_limits,self.shadow_hand_dof_upper_limits)
