...
class AllegroHandTrackingGeneralistDeploy(BaseTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        if os.path.exists(local_inst_tag_to_opt_state_fn): # local isnt tag and the glb inst tag # # data quality and the data coverage #
        ...
        # using all sup loss that is aboe the treshold as the trajecotr to providej suprvisions #
        ...
        # 'ring': 'link_11_tip' # link 3 tip # up axis #
        ...
        # 
        # set dof states in the simulator #
        # set dof states in the simulator # # do not add the ojbect poses or set object poses as well in the simulator #
...
# # >>>>>>> af613bc9b5b103fd64d3b7f92fe513073a2649c7
        ...
        for object_id, object_code in enumerate(self.object_code_list): # object_code 
            ...
            for obj_id, scale_id in object_scale_idx_pairs: # # find # # # since it is a dataset #
    ...
    def compute_observations(self): 
        # compute observations # # refresh dof state tensors #
        self.gym.refresh_dof_state_tensor(self.sim)
        self.gym.refresh_actor_root_state_tensor(self.sim)
        self.gym.refresh_rigid_body_state_tensor(self.sim)
    ...
    ### set hand dof states ##
    def set_hand_dof_states(self, hand_dof_states):
        # set the dof states to all environments # 
        # set hand dof states # 
        
        all_hand_indices = torch.unique(torch.cat([self.hand_indices]).to(torch.int32))
        # ahnd dof states #
        # acquire the dof states in the sims #
        
        # sset the dof state to hand replay qs # 
        # hand replay qs: nn_envs x nn_hand_dofs # 
        # nn_envs x 
        tot_envs_hand_dof_states = hand_dof_states.contiguous().unsqueeze(0).repeat(self.num_envs, 1)
        dof_state_cur = torch.zeros_like(self.dof_state)
        dof_state_cur[:, 0] = hand_dof_states.contiguous().view(-1).contiguous()
        dof_state_cur[:, 1] = 0.0
        # self.dof_state[:, 0] = cur_hand_replay_qs.contiguous().view(-1).contiguous() # get the dof state #
        # self.dof_state[:, 1] = 0.0
        self.gym.set_dof_state_tensor_indexed(self.sim, gymtorch.unwrap_tensor(dof_state_cur),
                                            gymtorch.unwrap_tensor(all_hand_indices), len(all_hand_indices))
        


        # do we need to set the dof states for objs? #
    ...
    def get_hand_dof_states(self, ):
        ...
        # 
        hand_dof_states = self.shadow_hand_dof_pos # torch tensor --- nn_envs x nn_dofs_states
        rigid_body_states = self.rigid_body_states # torch tnesor --- nn_envs x nn_bodies x nn_dof_states_body #

    # careate sims --- need to create it # --- some issues realted to teh paths # # feed the network actions here #
