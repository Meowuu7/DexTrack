...
class ShadowHandGrasp(BaseTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        # self.sim_params = sim_params # sim params #
        ...
        self.mocap_tracking = False
        # laod mocap sv info fn # 
        self.mocap_sv_info_fn = self.cfg['env']['mocap_sv_info_fn'] # get the mocap sv info fn #
        if len(self.mocap_sv_info_fn) > 0 and os.path.exists(self.mocap_sv_info_fn):
            self.mocap_tracking = True
            self._load_mocap_info() # load the mocap info from the mocap_sv_info_fn 
    ...
    def _load_mocap_info(self,):
        # goal_obj_trans_th, goal_obj_rot_quat_th #
        ...
        goal_obj_trans = save_info['object_transl'][:] #  # object transl -- and the objectjquat #
        ...
        print(f"==> Info loaded with goal_obj_trans: {goal_obj_trans.shape}, goal_obj_rot_quat: {goal_obj_rot_quat.shape}")
        ...
        self.goal_obj_rot_quat_th = torch.from_numpy(goal_obj_rot_quat).float().to(self.rl_device) # goal obj rot quat #
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if len(self.object_name) > 0: # get the object name #
            object_scale_dict = { f'sem/{self.object_name}' : [1.0] }
        
        
        self.object_code_list = list(object_scale_dict.keys()) # object code #
        all_scales = set()
        ...
        for object_code in self.object_code_list:
            ...
            #     dataset_root_path = "/home/xueyi/diffsim/UniDexGrasp/dexgrasp_policy/assets/datasetv4.1"
            #     dataset_path = dataset_root_path + '/' + object_code
            ...
            for num in data_num_list:
                ...
                assert scale in [0.06, 0.08, 0.10, 0.12, 0.15, 1.0]
        ...
        print(f"asset_root: {asset_root}, shadow_hand_asset_file: {shadow_hand_asset_file}")
        # load shadow hand_asset # load shadow hand asset #
        ...
        for object_id, object_code in enumerate(self.object_code_list):
            ...
            for obj_id, scale_id in object_scale_idx_pairs:
                if obj_id == object_id:
                    ...
                    mesh_path = "/home/xueyi/diffsim/UniDexGrasp/dexgrasp_policy/assets/meshdatav3_scaled"
                    # scaled_object_asset_file = "sem/ori_grab_s2_train_lift/coacd/coacd_1.urdf"
                    # scaled_object_asset_file = "sem/ori_grab_s2_phone_call_1/coacd/coacd_1.urdf"
                    # scaled_object_asset_file = "sem/ori_grab_s2_apple_lift/coacd/coacd_1.urdf"
                    # scaled_object_asset_file = "sem/ori_grab_s2_hammer_use_1/coacd/coacd_1.urdf"
                    scaled_object_asset_file = "sem/taco_20231104_203/coacd/coacd_1.urdf"
        ...
        table_z_height = 0.6
        # create table asset
        table_dims = gymapi.Vec3(1, 1, table_z_height)
        asset_options = gymapi.AssetOptions()
        ...
        # table_z_height # 
        shadow_hand_start_pose.p = gymapi.Vec3(0.0, 0.0, table_z_height + 0.2) 
        shadow_hand_start_pose.r = gymapi.Quat().from_euler_zyx(1.57, 0, 0)  # gymapi.Quat().from_euler_zyx(0, -1.57, 0)

        # table height #
        object_start_pose = gymapi.Transform()
        object_start_pose.p = gymapi.Vec3(0.0, 0.0, table_z_height + 0.1)  # gymapi.Vec3(0.0, 0.0, 0.72)
        ...
        # goal obj trans th, goal obj rot quat th #
        # # # goal_obj_trans_th, goal_obj_rot_quat_th # #  # 
        if self.mocap_tracking:
            self.goal_obj_trans_th[:, 2] = self.goal_obj_trans_th[:, 2] + table_z_height # add the table_z_height # 
    ...
    def compute_reward(self, actions, id=-1):
        ...
        # get the target goal pos # 
        if not self.mocap_tracking:
            goal_obj_pos = self.goal_pos
        else:
            # 
            clipped_progress_buf = torch.clamp(self.progress_buf, 0, max=self.goal_obj_trans_th.size(0) - 1)
            goal_obj_pos = self.goal_obj_trans_th[clipped_progress_buf.long()] # 
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.current_successes[:], self.consecutive_successes[:] = compute_hand_reward(
            ...
            goal_obj_pos, self.goal_rot, # goal obj pos #
