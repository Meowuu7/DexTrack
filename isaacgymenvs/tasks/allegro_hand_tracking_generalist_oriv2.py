...
class AllegroHandTrackingGeneralist(BaseTask):
    ...
    def _load_forcasting_model(self, ):
        ...
        if self.forcasting_diffusion_model:
            ...
            if self.hist_cond_partial_obj_pos_info:
                input_dims['cond_feat'] = 3
                output_dims['cond_feat'] = 3
            if self.hist_cond_partial_hand_info:
                input_dims['cond_feat'] = 22
                output_dims['cond_feat'] = 22
            if self.hist_cond_partial_obj_info:
                input_dims['cond_feat'] = 3 + 4
                output_dims['cond_feat'] = 3 + 4
    ...
    def _forward_forcasting_model(self, ):
        ...
        ### ws - 1, # whether they have been put into the hand and pos? ###
        ...
        forcasting_obj_pts_expanded = forcasting_obj_pts_expanded[:, -self.history_window_size:]
    ...
    def compute_observations(self):
        ...
        if self.use_forcasting_model and self.already_forcasted:
            ...
            cur_goal_rot = batched_index_select(self.forcast_obj_rot, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1) # nn_envs x 4 # # get the current forcasted obj rot
        ...
        if self.use_forcasting_model and self.already_forcasted:
            ...
            cur_goal_pos = batched_index_select(self.forcast_obj_pos, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1) # nn_envs x 3 # # get the current forcasted obj pos 
            cur_goal_rot = batched_index_select(self.forcast_obj_rot, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1) # nn_envs x 4 # # get the current forcasted obj rot # progress buf j# forcast obj rot #
    ...
    def compute_full_state(self, asymm_obs=False): #
        ...
        if self.use_future_obs:
            ...
            #### get the future goal obj pos and obj rot ####
            # # envs obj goal pos # # envs obj goal rot #
        ...
        if self.randomize_conditions:
            if self.condition_mask_type == MASK_OBJ:
                self.obs_buf[:, obj_obs_start: obj_obs_start + 16] = 0.0
        ...
        if self.obs_type == 'pure_state_wref_wdelta' and self.use_kinematics_bias_wdelta:
            ...
            # cur_hand_qpos_ref # the cur hand qpos #
        ...
        if not self.single_instance_state_based_test and not self.single_instance_state_based_train:
            ...
            if self.supervised_training:
                ...
                pass
        # print(f"[Debug] After computing full state")
        return
                ...
                for prop_name, prop_attrs in actor_properties.items():
                    ...
                    if prop_name == 'scale': # attribute randomization params #
                        ...
                        if attr_randomization_params['operation'] == 'scaling':
                            ...
                            new_scale = og_scale + sample
                        self.gym.set_actor_scale(env, handle, new_scale) # set the actor scale --- scale of the actors -- # scale #
                        continue
