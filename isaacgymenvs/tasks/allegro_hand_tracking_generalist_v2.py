...
class AllegroHandTrackingGeneralistV2(BaseTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        if len(self.preload_action_targets_fn) > 0 and os.path.exists(self.preload_action_targets_fn):
            if self.train_free_hand and self.tune_hand_pd:
                ...
                self.tot_action_targets = []
                for cur_ts in tot_tses:
                    cur_ts_action_targets = self.preload_action_targets[cur_ts]['shadow_hand_dof_tars'][self.preload_action_target_env_idx: self.preload_action_target_env_idx + 1]
                    self.tot_action_targets.append(cur_ts_action_targets)
    ...
    def _load_single_tracking_kine_info(self, data_inst_tag, cur_base_traj_fn=None):
        ...
        if isinstance(data_inst_tag, str):
            ...
            if self.w_franka:  # with 
                if self.load_kine_info_retar_with_arm and os.path.exists(self.kine_info_with_arm_sv_root):
                    ...
                    if 'link_key_to_link_pose' in kine_info_w_arm:
                        link_key_to_link_pose = kine_info_w_arm['link_key_to_link_pose']
                    else:
                        link_key_to_link_pose = None
                ...
                if 'link_key_to_link_pose' in cur_kine_data:
                    link_key_to_link_pose = cur_kine_data['link_key_to_link_pose']
                else:
                    link_key_to_link_pose = None
            ...
            if 'ori_grab' in obj_type:
                ...
                if self.w_franka: 
                    ...
                    if 'link_key_to_link_pose' in traj_kine_info_data:
                        link_key_to_link_pose = traj_kine_info_data['link_key_to_link_pose']
                    else:
                        link_key_to_link_pose = None
        ...
        if link_key_to_link_pose is not None:
            kine_info_dict['link_key_to_link_pose'] = link_key_to_link_pose
    ...
    def _prepare_expert_traj_infos(self,):
        ...
        for i_inst, cur_data_fn in enumerate(tot_data_fns):
            ...
            if 'link_key_to_link_pose' in cur_kine_data:
                link_key_to_link_pose = cur_kine_data['link_key_to_link_pose']
                for link_key, link_pose in link_key_to_link_pose.items():
                    if link_key not in tot_key_to_tot_link_pose:
                        tot_key_to_tot_link_pose[link_key] = []
                    tot_key_to_tot_link_pose[link_key].append(link_pose)
        ...
        if len(tot_key_to_tot_link_pose) > 0:
            self.tot_key_to_tot_link_pose = {}
            for link_key, link_pose in tot_key_to_tot_link_pose.items():
                self.tot_key_to_tot_link_pose[link_key] = torch.from_numpy(np.stack(link_pose, axis=0)).float().to(self.rl_device)
    ...
    def compute_reward_warm(self, actions, id=-1):
        ...
        if self.w_finger_pos_rew:
            ...
            if self.hand_type == 'allegro':
                ...
                palm_link_pose = self.tot_key_to_tot_link_pose['palm_link']
                thumb_link_pose =self.tot_key_to_tot_link_pose['link_15_tip'] # nn_insts x nn_frames x 3 # 
                index_link_pose =self.tot_key_to_tot_link_pose['link_3_tip'] # nn_insts x nn_frames x 3 # 
                middle_link_pose =self.tot_key_to_tot_link_pose['link_7_tip'] # nn_insts x nn_frames x 3 # 
                ring_link_pose =self.tot_key_to_tot_link_pose['link_11_tip'] # nn_insts x nn_frames x 3 # 
            elif self.hand_type == 'leap':
                palm_link_pos = self.tot_key_to_tot_link_pos['palm_lower'] # nn_insts x nn_frames x 3 # 
                thumb_link_pos =self.tot_key_to_tot_link_pos['thumb_tip_head'] # nn_insts x nn_frames x 3 # 
                index_link_pos =self.tot_key_to_tot_link_pos['index_tip_head'] # nn_insts x nn_frames x 3 # 
                ...
                palm_link_pose = self.tot_key_to_tot_link_pose['palm_lower']
                thumb_link_pose =self.tot_key_to_tot_link_pose['thumb_tip_head'] # nn_insts x nn_frames x 3 # 
                index_link_pose =self.tot_key_to_tot_link_pose['index_tip_head'] # nn_insts x nn_frames x 3 # 
                middle_link_pose =self.tot_key_to_tot_link_pose['middle_tip_head'] # nn_insts x nn_frames x 3 # 
                ring_link_pose =self.tot_key_to_tot_link_pose['ring_tip_head'] # nn_insts x nn_frames x 3 # 
            else:
                raise ValueError(f"Invalid hand type: {self.hand_type}")
            ...
            palm_link_pose = batched_index_select(palm_link_pose, self.env_inst_idxes, dim=0)
            thumb_link_pose = batched_index_select(thumb_link_pose, self.env_inst_idxes, dim=0)
            index_link_pose = batched_index_select(index_link_pose, self.env_inst_idxes, dim=0)
            middle_link_pose = batched_index_select(middle_link_pose, self.env_inst_idxes, dim=0)
            ring_link_pose = batched_index_select(ring_link_pose, self.env_inst_idxes, dim=0) # nn_envs x nn_frames x 3 #
            
            palm_link_pose = batched_index_select(palm_link_pose, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1)
            thumb_link_pose = batched_index_select(thumb_link_pose, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1)
            index_link_pose = batched_index_select(index_link_pose, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1)
            middle_link_pose = batched_index_select(middle_link_pose, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1)
            ring_link_pose = batched_index_select(ring_link_pose, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1)
            # print(f"[Debug] w_finger_pos_rew")
        else:
            palm_link_pos = self.right_hand_pos
            thumb_link_pos = self.right_hand_ff_pos
            ...
            palm_link_pose = torch.cat(
                [ self.right_hand_pos, self.right_hand_rot ], dim=-1
            )
            thumb_link_pose = torch.cat( [self.right_hand_th_pos, self.right_hand_th_rot] , dim=-1)
            index_link_pose = torch.cat( [self.right_hand_ff_pos, self.right_hand_ff_rot] , dim=-1)
            middle_link_pose = torch.cat( [self.right_hand_mf_pos, self.right_hand_mf_rot] , dim=-1)
            ring_link_pose = torch.cat( [self.right_hand_rf_pos, self.right_hand_rf_rot] , dim=-1)
        
        palm_link_rot = palm_link_pose[..., 3:]
        thumb_link_rot = thumb_link_pose[..., 3:]
        index_link_rot = index_link_pose[..., 3:]
        middle_link_rot = middle_link_pose[..., 3:]
        ring_link_rot = ring_link_pose[... , 3:]
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.current_successes[:], self.consecutive_successes[:] = compute_reward_func( 
            ...
            palm_link_pos, thumb_link_pos, index_link_pos, middle_link_pos, ring_link_pos,
            self.right_hand_rot, self.right_hand_th_rot, self.right_hand_ff_rot, self.right_hand_mf_rot, self.right_hand_rf_rot,
            palm_link_rot, thumb_link_rot, index_link_rot, middle_link_rot, ring_link_rot,
            self.dist_reward_scale, self.rot_reward_scale, self.rot_eps, self.actions, self.action_penalty_scale,
            self.success_tolerance, self.reach_goal_bonus, self.fall_dist, self.fall_penalty,
    ...
    def compute_observations(self):
        ...
        if self.test:
            ...
            self.ts_to_hand_obj_states[self.ref_ts] = {
                ...
                'goal_pose_ref_np': self.goal_pose_ref_np,
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        if not self.use_history_obs:
            ...
            if self.use_local_canonical_state:
                ...
                # print(f"using local canonicalizations")
                # canon_shadow_hand_dof_trans = self.shadow_hand_dof_pos[..., :3] - self.object_pos[..., :]
                canon_shadow_hand_dof_trans = torch.zeros((self.shadow_hand_dof_pos.size(0), 7), dtype=torch.float32).to(self.rl_device)
                canon_shadow_hand_dof = torch.cat(
            ...
            if self.use_local_canonical_state:
                canon_right_hand_pos = self.right_hand_pos - self.right_hand_pos # .unsqueeze(1) # self.object_pos
            else:
                canon_right_hand_pos = self.right_hand_pos
            ...
            if self.use_local_canonical_state:
                # canon_object_pos = self.object_pos - self.object_pos
                canon_object_pos = self.object_pos - self.right_hand_pos # .unsqueeze(1)
            else:
                canon_object_pos = self.object_pos  
        ...
        if self.obs_type == 'pure_state_wref_wdelta' and self.use_kinematics_bias_wdelta:
            ...
            if self.only_use_hand_first_frame:
                ...
                if self.use_local_canonical_state: # nex_hand_qpos_ref #
                    # canon_hand_qpos_trans = self.nex_hand_qpos_ref[..., :3] - self.object_pos
                    # canon_hand_
                    canon_hand_qpos_trans = torch.zeros((self.nex_hand_qpos_ref.size(0), 7), dtype=torch.float32).to(self.rl_device)
                    canon_hand_qpos_ref = torch.cat(
...
def compute_hand_reward_tracking_warm(
        ...
        right_hand_pos, right_hand_ff_pos, right_hand_mf_pos, right_hand_rf_pos, right_hand_th_pos,
        palm_link_pos, thumb_link_pos, index_link_pos, middle_link_pos, ring_link_pos,
        right_hand_rot, right_hand_th_rot, right_hand_ff_rot, right_hand_mf_rot, right_hand_rf_rot,
        palm_link_rot, thumb_link_rot, index_link_rot, middle_link_rot, ring_link_rot,
        dist_reward_scale: float, rot_reward_scale: float, rot_eps: float,
        actions, action_penalty_scale: float,
        success_tolerance: float, reach_goal_bonus: float, fall_dist: float,
    ...
    if w_finger_pos_rew:
        ...
        ####### Newly added in v2 #######
        palm_link_quat_diff = quat_mul(right_hand_rot, quat_conjugate(palm_link_rot))
        palm_link_rot_dist = 2.0 * torch.asin(torch.clamp(torch.norm(palm_link_quat_diff[:, 0:3], p=2, dim=-1), max=1.0))
        palm_link_rot_dist_coef = 1.0 * cur_glb_penalty_coef
        diff_palm_link_pos = diff_palm_link_pos + palm_link_rot_dist * palm_link_rot_dist_coef
        ####### Newly added in v2 #######
    ...
    if add_global_motion_penalty:
        ...
        ## add the global action penalty ies # 
