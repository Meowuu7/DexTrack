...
def compute_hand_reward(
    ...
    if goal_cond:
        ...
        hand_up = torch.zeros_like(right_hand_finger_dist)
        hand_up = torch.where(lowest >= 1.10, torch.where(flag == 2, 0.1 + 0.1 * actions[:, 2], hand_up), hand_up)
        hand_up = torch.where(lowest >= 1.30, torch.where(flag == 2, 0.2 - goal_hand_dist * 0, hand_up), hand_up)
