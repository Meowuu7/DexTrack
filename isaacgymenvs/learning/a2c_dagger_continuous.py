...
from isaacgymenvs.learning.storage import RolloutStorage
...
class A2CBase(BaseAlgorithm):
    ...
    # for the expert the network is actor critic #
    ...
    def set_full_state_weights(self, weights, set_epoch=True): # full state weights #
...
class ContinuousA2CBaseDAGGER(A2CBase):
    ...
    def __init__(self, base_name, params):
        ...
        teacher_obs_shape = deepcopy(obs_shape)
        teacher_obs_shape = [teacher_obs_dim] + list(teacher_obs_shape[1:])
        teacher_obs_shape = tuple(teacher_obs_shape)
        print(f"teacher_obs_shape: {teacher_obs_shape}, obs_shape: {obs_shape}")
        build_config = {
            'actions_num' : self.actions_num,
            'input_shape' : teacher_obs_shape, # obs_shape,
        ...
        ### export actor -- self.mdoel.actor ###
        ### student actor -- shoudl build with a differetnt one #
        ## TODO: use the algo factory to build the student network --- using a different st of params perhaps ## 
        ## 
        self.model = self.config['network'].build(build_config).to(self.device)
        print(f"Start loading student network")
        self.student_network_config = deepcopy(params)
        self.student_network_config['model'] = deepcopy(params['student_model'])
        self.student_network_config['network'] = deepcopy(params['student_network'])
        self.load_student_networks(self.student_network_config)
        
        student_build_config = {
            'actions_num' : self.actions_num,
            'input_shape' : obs_shape, # obs_shape,
            'num_seqs' : self.num_actors * self.num_agents,
            'value_size': self.env_info.get('value_size',1),
            'normalize_value' : self.normalize_value,
            'normalize_input': self.normalize_input,
        ...
        self.student_model = self.config['student_network'].build(student_build_config).to(self.device)
        
        ## TODO: add to the storage ## 
        self.load_network_ckpt(params)
        self.expert_ckpt = params['load_path']
        
        

        ## TODO: add the storage ##
        sampler = 'sequential'
        self.buffer_size = params['DAgger_learning_params']['buffer_size']
        self.state_space_shape = teacher_obs_shape
        self.action_space_shape = (self.actions_num, )
        self.storage = RolloutStorage(self.vec_env.env.num_envs, self.buffer_size, self.observation_space.shape,
                                      self.state_space_shape, self.action_space_shape, self.device, sampler)
        ## TODO: add the opimizer ##
        dagger_training_params = params['DAgger_learning_params']
        self.optimizer = optim.Adam(self.student_model.parameters(), lr=dagger_training_params['learning_rate'])

        # DAGGER parameters #
        self.num_learning_epochs = dagger_training_params['num_learning_epochs']
        self.num_mini_batches = dagger_training_params['num_mini_batches']
        self.num_transitions_per_env = dagger_training_params['num_transitions_per_env']

        # Log
        self.log_dir =  dagger_training_params['log_dir']
        self.print_log = dagger_training_params['print_log']
        self.writer = SummaryWriter(log_dir=self.log_dir, flush_secs=10)
        self.tot_timesteps = 0
        self.tot_time = 0
        self.is_testing = params['test']
        self.current_learning_iteration = 0

        self.apply_reset = False

        ## TODO: load the expert checkpoint ## 
    ...
    def restore(self, ckpt_path):
        ...
        # self.current_learning_iteration = ckpt_dict['current_learning_iteration']
        

    def restore_expert_model(self,):
        ckpt_weights = torch.load(self.expert_ckpt, map_location='cpu')
        model_weights = ckpt_weights['model']
        self.model.load_state_dict(model_weights)
        # self.model.restore(self.expert_ckpt)
    ...
    def load_network_ckpt(self, args):
        ...
        ckpt_weights = torch.load(args['load_path'], map_location='cpu')
        model_weights = ckpt_weights['model']
        print(f"self.model: {self.model}")
        self.model.load_state_dict(model_weights)
        # self.model.restore(args['checkpoint'])

        # for the expert the network is actor critic #
    ## TODO: load student network here ##
    def load_student_networks(self, params):
        builder = model_builder.ModelBuilder()
        self.config['student_network'] = builder.load(params)
        has_central_value_net = self.config.get('central_value_config') is not  None
        if has_central_value_net:
            print('Adding Central Value Network')
            if 'model' not in params['config']['central_value_config']:
    ...
    ## TODO: blend the run function with the train function ##
    def run(self, num_learning_iterations, log_interval=1):
        # teacher -- 
        # model_name -> continuous_a2c_logstd
        # network_name -> actor_critic
        id = -1
        if self.is_testing:
            self.vec_env.env.random_time = False
        current_obs = self.vec_env.reset()
        # current_states = self.vec_env.get_state()

        if self.is_testing: # is testing #
            length1=self.vec_env.env.max_episode_length
            for _ in range(length1):
                with torch.no_grad():
                    # Compute the action ## learning iterations #
                    id = (id+1)%self.vec_env.env.task.max_episode_length
                ...
                if _ == length1-2:
                    success_rate=self.vec_env.env.task.successes.sum()/self.vec_env.num_envs
            print("success_rate:",success_rate)
            
        else:
            # expert #
            self.restore_expert_model()
            self.model.eval()
            # self.actor_expert.load_state_dict(torch.load(self.expert_ckpt_path, map_location = self.device)) 
            # self.actor_expert.eval()

            rewbuffer = deque(maxlen=100)
            lenbuffer = deque(maxlen=100)
            cur_reward_sum = torch.zeros(self.vec_env.env.num_envs, dtype=torch.float, device=self.device)
            cur_episode_length = torch.zeros(self.vec_env.env.num_envs, dtype=torch.float, device=self.device)
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                self.student_prev_actions = torch.zeros((self.vec_env.env.num_envs, self.actions_num), dtype=torch.float, device=self.device)
                self.expert_prev_actions = torch.zeros_like(self.student_prev_actions)
                ...
                for _ in range(self.num_transitions_per_env):
                    if self.apply_reset:
                        current_obs = self.vec_env.reset()
                    id = (id+1) % self.vec_env.env.max_episode_length
                    
                    # in the first running --- no previous actions? # 
                    # calculate input actions #
                    # 
                    # batch_input_dict = {
                    #     'is_train': not self.is_testing,
                    #     'obs': current_obs,
                    #     'prev_actions': self.prev_actions
                    # }
                    
                    current_obs = self._preproc_obs(current_obs)
                    
                    student_input_dict = {
                        'prev_actions': self.student_prev_actions,
                        'is_train': False , # not self.is_testing,
                        'obs': current_obs,
                    }
                    expert_input_dict = {
                        'prev_actions': self.expert_prev_actions,
                        'is_train': False , # not self.is_testing,
                        'obs': current_obs[..., : self.teacher_obs_dim],
                    }
                    
                    # running mean #
                    # results = self.student_model(current_obs)
                    # results_expert = self.model(current_obs)
                    
                    
                    results = self.student_model(student_input_dict)
                    results_expert = self.model(expert_input_dict)
                    
                    # print(f"results: {results}")
                    # print(f"results: {results.keys()}")
                    
                    actions = results['actions']
                    actions_expert = results_expert['actions']
                    
                    self.student_prev_actions = actions
                    self.expert_prev_actions = actions_expert
                    ...
                    if self.print_log: # print log # print log # pring log #
                        cur_reward_sum[:] += rews
                ...
                # current_obs = 
                _ = self.student_model(cur_input)
                stop = time.time()
    ...
    def log(self, locs, width=80, pad=35):
        ...
        fps = int(self.num_transitions_per_env * self.vec_env.env.num_envs / (locs['collection_time'] + locs['learn_time']))
    ...
    def update(self):
        ...
        for epoch in range(self.num_learning_epochs):
            for indices in batch:
                ...
                student_input_dict = {
                    'prev_actions': torch.zeros((obs_batch.size(0), self.actions_num), dtype=torch.float, device=self.device),
                    'obs': obs_batch,
                    'is_train': True
                }
                ...
                student_results = self.student_model(student_input_dict)
                
                mu_, sigma_ = student_results['mus'], student_results['sigmas']
                # distr = torch.distributions.Normal(mu_, sigma_, validate_args=False)
                # nn_envs x nn_acts# 
                noise_ = torch.randn_like(sigma_)
                actions_batch = mu_ + sigma_ * noise_
                
                # actions_batch = student_results['actions']
    ...
    def train_epoch(self):
        self.run(self.max_epochs, log_interval=1)
