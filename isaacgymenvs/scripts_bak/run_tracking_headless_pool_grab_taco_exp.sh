...
export excluded_fr_existing_res_fn='/cephfs/xueyi/uni_manip/isaacgym_rl_exp_taco_grab_interpseq/statistics/obj_type_to_optimized_res_beta.npy'
export excluded_fr_existing_res_fn='/cephfs/xueyi/uni_manip/isaacgym_rl_exp_taco_grab_interpseq_interpfr_60_interpfr2_60_nntrans_40/statistics/obj_type_to_optimized_res_beta_v1.npy'
# export excluded_fr_existing_res_fn=''


# export log_path='./runs_init_fr_samples'
# export log_path='/cephfs/xueyi/uni_manip/isaacgym_rl_exp_taco_grab_interpseq_remaining_temp'
export log_path='/cephfs/xueyi/uni_manip/isaacgym_rl_exp_taco_grab_interpseq_interpfr_60_interpfr2_60_nntrans_40_remaining_temp'

export st_idx=0


# but no changes current for the alreadly launched exps

# export target_mocap_sv_fn='' 

# bash scripts/run_tracking_headless_pool_grab_taco_exp.sh

CUDA_VISIBLE_DEVICES=${cuda_idx} python train_pool_taco_grab.py --additional_tag=${additional_tag} --hand_type='allegro' --numEnvs=${numEnvs} --minibatch_size=${minibatch_size} --goal_cond=${goal_cond}  --test=${test} --use_relative_control=${use_relative_control} --use_kinematics_bias=${use_kinematics_bias} --w_obj_ornt=${w_obj_ornt} --obs_type=${obs_type} --separate_stages=${separate_stages} --rigid_obj_density=${rigid_obj_density}  --kinematics_only=${kinematics_only} --use_fingertips=${use_fingertips}  --use_kinematics_bias_wdelta=${use_kinematics_bias_wdelta} --hand_pose_guidance_glb_trans_coef=${hand_pose_guidance_glb_trans_coef} --hand_pose_guidance_glb_rot_coef=${hand_pose_guidance_glb_rot_coef} --hand_pose_guidance_fingerpose_coef=${hand_pose_guidance_fingerpose_coef} --dt=${dt} --glb_trans_vel_scale=${glb_trans_vel_scale} --glb_rot_vel_scale=${glb_rot_vel_scale} --rew_finger_obj_dist_coef=${rew_finger_obj_dist_coef} --rew_delta_hand_pose_coef=${rew_delta_hand_pose_coef} ${debug} --nn_gpus=${nn_gpus} --num_frames=${num_frames} --tracking_data_sv_root=${tracking_data_sv_root} --subj_nm=${subj_nm}  --st_idx=${st_idx} --dofSpeedScale=${dofSpeedScale} --use_twostage_rew=${use_twostage_rew} --episodeLength=${episodeLength} --data_inst_flag=${data_inst_flag} --target_object_name=${target_object_name} --target_mocap_sv_fn=${target_mocap_sv_fn} --use_taco_obj_traj=${use_taco_obj_traj} --exp_type=${exp_type} --taco_interped_data_sv_additional_tag=${taco_interped_data_sv_additional_tag} --excluded_fr_existing_res_fn=${excluded_fr_existing_res_fn} --log_path=${log_path}
