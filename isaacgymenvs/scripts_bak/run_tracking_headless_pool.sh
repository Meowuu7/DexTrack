...
export checkpoint='runs/tracking_taco_20231104_203_obs_pure_state_density_500.0_trans_7.0_rot_1.0_goalcond_False_kinebias_t0.5r0.5f20_rfd_0.3_rh_0.5_12-21-55-30/nn/tracking_taco_20231104_203_obs_pure_state_density_500.0_trans_7.0_rot_1.0_goalcond_False_kinebias_t0.5r0.5f20_rfd_0.3_rh_0.5.pth'
...
# export data_inst_flag="taco_20231104_203"
export data_inst_flag="taco_20231104_114"
export glb_trans_vel_scale=7
export glb_rot_vel_scale=1
export glb_trans_vel_scale=3
export glb_rot_vel_scale=10
export glb_trans_vel_scale=7
export glb_rot_vel_scale=10
...
export glb_rot_vel_scale=0.5
export dofSpeedScale=20
# export glb_trans_vel_scale=2
# export glb_rot_vel_scale=2
# export dofSpeedScale=40
export glb_trans_vel_scale=7
export glb_rot_vel_scale=0.5
...
export goal_dist_thres=0.5

#####  hand guidance only #####
export rew_obj_pose_coef=0.0
export rew_finger_obj_dist_coef=0.0
export rew_delta_hand_pose_coef=0.5
export additional_tag=kinebias_interpeddata_stfr${start_frame}_t${glb_trans_vel_scale}r${glb_rot_vel_scale}f${dofSpeedScale}_rfd_${rew_finger_obj_dist_coef}_rh_${rew_delta_hand_pose_coef}_robj_${rew_obj_pose_coef}
#####  hand guidance only #####
...
# export data_inst_flag="taco_20231104_111"
export additional_tag=kinebias_interpeddata_stfr${start_frame}_t${glb_trans_vel_scale}r${glb_rot_vel_scale}f${dofSpeedScale}_rfd_${rew_finger_obj_dist_coef}_rh_${rew_delta_hand_pose_coef}_robj_${rew_obj_pose_coef}_gd_${goal_dist_thres}
export lifting_separate_stages=False
...
# dat ains flag 
export data_inst_flag='taco_20231104_203' # 
export disable_gravity=True # discable gravity 
...
#### separate lifting stage with the strict separate lifting stage ####
# # export goal_dist_thres=0.2 
# export goal_dist_thres=0.4
# export rew_finger_obj_dist_coef=0.5
# export rew_delta_hand_pose_coef=0.0
# export rew_obj_pose_coef=1.0
...
# export data_inst_flag="taco_20231104_203"
# export data_inst_flag="taco_20231104_016"
# export additional_tag=kinebias_interpeddata_stfr${start_frame}_t${glb_trans_vel_scale}r${glb_rot_vel_scale}f${dofSpeedScale}_rfd_${rew_finger_obj_dist_coef}_rh_${rew_delta_hand_pose_coef}_robj_${rew_obj_pose_coef}_gd_${goal_dist_thres}_liftingsep${lifting_separate_stages}
...
### separate stage with a large speed ###
# export glb_trans_vel_scale=7
# export glb_rot_vel_scale=0.5
# export separate_stages=True
# export additional_tag=kinebias_interpeddata_t${glb_trans_vel_scale}r${glb_rot_vel_scale}f${dofSpeedScale}_rfd_${rew_finger_obj_dist_coef}_rh_${rew_delta_hand_pose_coef} # 
### separate stage with a large speed ###
...
# export tracking_data_sv_root='/home/xueyi/IsaacGymEnvs/assets/retar_data'
# export st_idx=0
# export add_table=True
# export table_z_dim=0.5
########## Capture video setting ##########
...
# bash scripts/run_tracking_headless_pool.sh


CUDA_VISIBLE_DEVICES=${cuda_idx} python train_pool.py --additional_tag=${additional_tag} --hand_type='allegro' --numEnvs=${numEnvs} --minibatch_size=${minibatch_size} --goal_cond=${goal_cond}  --test=${test} --use_relative_control=${use_relative_control} --use_kinematics_bias=${use_kinematics_bias} --w_obj_ornt=${w_obj_ornt} --obs_type=${obs_type} --separate_stages=${separate_stages} --rigid_obj_density=${rigid_obj_density}  --kinematics_only=${kinematics_only} --use_fingertips=${use_fingertips}  --use_kinematics_bias_wdelta=${use_kinematics_bias_wdelta} --hand_pose_guidance_glb_trans_coef=${hand_pose_guidance_glb_trans_coef} --hand_pose_guidance_glb_rot_coef=${hand_pose_guidance_glb_rot_coef} --hand_pose_guidance_fingerpose_coef=${hand_pose_guidance_fingerpose_coef} --dt=${dt} --glb_trans_vel_scale=${glb_trans_vel_scale} --glb_rot_vel_scale=${glb_rot_vel_scale} --rew_finger_obj_dist_coef=${rew_finger_obj_dist_coef} --rew_delta_hand_pose_coef=${rew_delta_hand_pose_coef} ${debug} --nn_gpus=${nn_gpus} --num_frames=${num_frames} --tracking_data_sv_root=${tracking_data_sv_root} --subj_nm=${subj_nm}  --st_idx=${st_idx} --dofSpeedScale=${dofSpeedScale} --use_twostage_rew=${use_twostage_rew} --dataset_type=${dataset_type} --ground_distance=${ground_distance} --use_canonical_state=${use_canonical_state} --disable_gravity=${disable_gravity} --data_inst_flag=${data_inst_flag} --right_hand_dist_thres=${right_hand_dist_thres} --checkpoint=${checkpoint} --max_epochs=${max_epochs} --use_real_twostage_rew=${use_real_twostage_rew} --start_grasping_fr=${start_grasping_fr} --controlFrequencyInv=${controlFrequencyInv} --use_interpolated_data=${use_interpolated_data} --episodeLength=${episodeLength} --start_frame=${start_frame} --rew_obj_pose_coef=${rew_obj_pose_coef} --goal_dist_thres=${goal_dist_thres} --lifting_separate_stages=${lifting_separate_stages} --strict_lifting_separate_stages=${strict_lifting_separate_stages} --add_table=${add_table} --table_z_dim=${table_z_dim} --headless=${headless}
