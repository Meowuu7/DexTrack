...
class LeapHandArm:
	def __init__(self, p_gain=10.0, d_gain=0.1, dt=0.0167, rl_device='cuda:0', sim_device='cuda:0',
		...
		self.robot_asset_files_dict = {
			...
			# "leap": "urdf/franka_description/robots/franka_panda_leaphand.urdf",
			"leap": "leap_hand/franka_panda_leaphand.urdf",
		...
		self.p_gain_val_hand = p_gain
		self.d_gain_val_hand = d_gain
		self.p_gain = torch.ones((self.num_envs, self.num_actions), device=self.device, dtype=torch.float) * self.p_gain_val_hand
		self.d_gain = torch.ones((self.num_envs, self.num_actions), device=self.device, dtype=torch.float) * self.d_gain_val_hand
	...
	def set_sim_params(self, dt):
		...
		sim_params.substeps = 2 			# config_sim.get("substeps", 2)
	...
	def create_envs(self, num_envs, spacing, num_per_row):
		...
		arm_hand_asset_file = self.robot_asset_files_dict["leap"]
		...
		for i in range(23):
			...
			if not self.torque_control:
				print(f"torque_control: {self.torque_control}")
		...
		self.default_arm_pos = [0, -0.7853, 0, -2.35539, 0, 1.57, 0.785]
	...
	def reset_idx(self, env_ids):
		# reset the pd-gain.
		self.randomize_p_gain_lower = self.p_gain_val_hand - 0.03
		self.randomize_p_gain_upper = self.p_gain_val_hand + 0.03
		self.randomize_d_gain_lower = self.d_gain_val_hand - 0.03
		self.randomize_d_gain_upper = self.d_gain_val_hand + 0.03
		...
		self.randomize_p_gain_lower = self.p_gain_val_arm_0123 - 0.03
		self.randomize_p_gain_upper = self.p_gain_val_arm_0123 + 0.03
		self.randomize_d_gain_lower = self.d_gain_val_arm_0123 - 0.03
		self.randomize_d_gain_upper = self.d_gain_val_arm_0123 + 0.03
		...
		self.randomize_p_gain_lower = self.p_gain_val_arm_4 - 0.03
		self.randomize_p_gain_upper = self.p_gain_val_arm_4 + 0.03
		self.randomize_d_gain_lower = self.d_gain_val_arm_4 - 0.03
		self.randomize_d_gain_upper = self.d_gain_val_arm_4 + 0.03
		...
		self.randomize_p_gain_lower = self.p_gain_val_arm_5 - 0.03
		self.randomize_p_gain_upper = self.p_gain_val_arm_5 + 0.03
		self.randomize_d_gain_lower = self.d_gain_val_arm_5 - 0.03
		self.randomize_d_gain_upper = self.d_gain_val_arm_5 + 0.03
		new_values = torch_rand_float(self.randomize_p_gain_lower, self.randomize_p_gain_upper, (len(env_ids), 1), device=self.device)
		self.p_gain[env_ids, 5] = new_values.squeeze(1)

		self.randomize_p_gain_lower = self.p_gain_val_arm_6 - 0.03
		self.randomize_p_gain_upper = self.p_gain_val_arm_6 + 0.03
		self.randomize_d_gain_lower = self.d_gain_val_arm_6 - 0.03
		self.randomize_d_gain_upper = self.d_gain_val_arm_6 + 0.03
		new_values = torch_rand_float(self.randomize_p_gain_lower, self.randomize_p_gain_upper, (len(env_ids), 1), device=self.device)
	...
	def goto_qpos(self, qpos, control_steps):
		...
		self.cur_targets[:, self.actuated_dof_indices] = tensor_clamp(qpos,
																	  self.arm_hand_dof_lower_limits[self.actuated_dof_indices],
																	  self.arm_hand_dof_upper_limits[self.actuated_dof_indices])
		for i in range(control_steps * 3):
			if self.force_render:
				self.render()
...
class ImpulseFunc:
	def __init__(self, shift, horizon = 60):
		self.shift = shift
...
class SinWaveFunc:
	...
	def __call__(self, x):
		import math
		all_time_steps = []
		delta_shifts = []  # 用于存储每个时间步之间的 Δshift
		prev_shift = 0.0  # 初始化前一个时间步的 shift 为 0
		for t in range(self.horizon):
			...
			# 计算 Δshift，跳过第一个时间步
			if t > 0:
				delta_shift = shift - prev_shift
				delta_shifts.append(delta_shift)
				print(f"Delta shift at time step {t}: {delta_shift:.4f}")
...
if __name__ == '__main__':
	...
	env = LeapHandArm(p_gain=100.0, d_gain=4.0) # initial 100,4
	...
	# all_tests = [(init_qpos, ImpulseFunc(0.3, horizon=control_num)),
	# 			 (init_qpos, SinWaveFunc(0.1, T=2.0, dt=0.05, horizon=control_num)), (init_qpos, SinWaveFunc(0.05, T=0.5, dt=0.05, horizon=control_num)) #(init_qpos, SinWaveFunc(0.1, T=0.3, horizon=60))
	# 			 ]
 
	all_tests = [(init_qpos, ImpulseFunc(0.3, horizon=control_num)),
				 (init_qpos, SinWaveFunc(0.1, T=2.0, dt=0.05, horizon=control_num)), (init_qpos, SinWaveFunc(0.1, T=0.5, dt=0.05, horizon=control_num)) #(init_qpos, SinWaveFunc(0.1, T=0.3, horizon=60))
				 ]

	real_to_sim_indices = [1, 0, 2, 3, 12, 13, 14, 15, 5, 4, 6, 7, 9, 8, 10, 11]

	# actual_impulse_trajectory = np.load('/home/janebek/Janebek/ws_leaphand/src/ros_module/trajectory/actual_impulse_trajectory.npy')
	# actual_impulse_trajectory = actual_impulse_trajectory[:, real_to_sim_indices]

 
	actual_sinwave_sv_fn = '/home/xymeow/xueyi/leaphand_ws/actual_sinwave_trajectory.npy'
	# actual_sinwave_trajectory = np.load('/home/janebek/Janebek/ws_leaphand/src/ros_module/trajectory/actual_sinwave_trajectory.npy')
	actual_sinwave_trajectory = np.load(actual_sinwave_sv_fn)
	actual_sinwave_trajectory = actual_sinwave_trajectory[:, real_to_sim_indices]

	# actual_franka_sinwave_trajectory = np.load('/home/janebek/Janebek/deoxys_control/actual_franka_sinwave_trajectory.npy')
	...
	for test_idx, test in enumerate(all_tests):
		...
		env.goto_qpos(test_init_qpos, control_num)

		results = env.goto_qpos_seq(target_qpos, control_num)
		...
		# results, target_qpos #
        # leaphand calibration
		for i in range(16):
			ax = axs[i // 4, i % 4]
			ax.plot(np.linspace(0, horizon - 1, horizon) * 0.05, results[:, i, i + 7], 'b-', label="sim_tracking")
			ax.plot(np.linspace(0, horizon - 1, horizon) * 0.05, target_qpos[i, :, i + 7].cpu().detach().numpy(), 'r--', label="target")
			if not impulse_flag:
				ax.plot(np.linspace(0, horizon - 1, horizon) * 0.05, actual_sinwave_trajectory[:, i], 'g-', label="real_tracking")
			else:
				ax.plot(np.linspace(0, horizon - 1, horizon) * 0.05, actual_sinwave_trajectory[:, i], 'g-', label="real_tracking")				
			ax.set_title(finger_mapping_dict[i + 7])
			ax.set_xlabel('Time (s)')
			ax.set_ylabel('Position (rad)')
			ax.grid(True)
			ax.legend()

		# # franka panda calibration
		# for i in range(7):
		# 	ax = axs[i // 4, i % 4]
		# 	ax.plot(np.linspace(0, horizon - 1, horizon) * 0.05, results[:, i, i ], 'b-', label="sim_tracking")
		# 	ax.plot(np.linspace(0, horizon - 1, horizon) * 0.05, target_qpos[i, :, i ].cpu().detach().numpy(), 'r--', label="target")
		# 	ax.plot(np.linspace(0, horizon - 1, horizon) * 0.05, actual_franka_sinwave_trajectory[:, i], 'g-', label="real_tracking")			
		# 	ax.set_title(i+1)
		# 	ax.set_xlabel('Time (s)')
		# 	ax.set_ylabel('Position (rad)')
		# 	ax.grid(True)
		...
		plt.tight_layout()
		plt.show()  # 只在需要显示图形时调用
		impulse_flag = True

		# plt.tight_layout() #
		# plt.pause(5)  # 减少暂停时间
		# plt.close(fig)  # 确保每次迭代结束时关闭图形窗口
	pass
