...
class Transformer_Net_PC_Seq_V3_AE(nn.Module):
    def __init__(self, n_layers: int, input_dims: dict, hidden_mlp_dims: dict,
                 output_dims: dict, act_fn_in: nn.ReLU(), act_fn_out: nn.ReLU(), traj_cond=False, task_cond_type='future', sub_task_cond_type='full', debug=False):
        super().__init__()
...
## task cond configs v1 ---- including what? ##
### mlp net for processing point cloud data ###
...
class Transformer_Net_PC_Seq_V3_KineDiff_AE(nn.Module):
    def __init__(self, n_layers: int, input_dims: dict, hidden_mlp_dims: dict,
        ...
        self.activation = 'relu'
        self.num_layers = n_layers
        self.traj_cond = traj_cond ## trajectory conditional argument ##
...
class InputProcessObjBaseV5(nn.Module):
    ...
    def forward(self, x, rt_glb=False, permute_dim=True): # decode relative positions and others ##
        ...
        x_pts_emb = self.pts_feats_encoding_net(
            x # pts feats encoding net --- x_pts_emb # 
        )
