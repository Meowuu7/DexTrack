...
class LandEnvironment(BaseEnv):
    ...
    def reset(self, design=None):
        ...
        if self.cfg.ENVIRONMENT.CUSTOM.has_matter_on_ground:
            ...
            self.reset_matter(reset_s, matter_material_id) # reset matter 
    ...
    def get_max_surface_height_under_robot(self, s: I_DTYPE) -> F_DTYPE:
        ...
        for p in range(self.sim.solver.n_particles[None]):
            id = self.sim.solver.particle_ids[p] # get the particle
            if self.design_space.is_robot(id) and self.sim.solver.p_rho[p] > 0:
                ...
                i = ti.min(ti.max(I[0], 0), self.terrain_info['resolution'] - 1) # resolution 
                j = ti.min(ti.max(I[2], 0), self.terrain_info['resolution'] - 1) # resolution 
                surface_point = self.terrain_info['polysurface_points'][i, j] # terrion information; furface point 
