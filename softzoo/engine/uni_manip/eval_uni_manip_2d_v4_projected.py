...
def get_rotational_translations_vels_accs(transformation_vecs, dt=1e-1):
    ### 
    nn_timesteps = transformation_vecs.shape[0]
    transformation_vels = []
    transformation_accs = []
    for i_ts in range(0, nn_timesteps):
        cur_transformation_vec = transformation_vecs[i_ts]
        
        if i_ts == 0:
            cur_transformation_vel = np.zeros_like(cur_transformation_vec)
            cur_transformation_acc = np.zeros_like(cur_transformation_vec)
        else:
            prev_transformation_vec = transformation_vecs[i_ts - 1]
            cur_transformation_vel = (cur_transformation_vec - prev_transformation_vec) / dt
            prev_transformation_vel = transformation_vels[-1]
            cur_transformation_acc = (cur_transformation_vel - prev_transformation_vel) / dt
        transformation_vels.append(cur_transformation_vel)
        transformation_accs.append(cur_transformation_acc)
    transformation_vels = np.stack(transformation_vels, axis=0)
    transformation_accs = np.stack(transformation_accs, axis=0) 
    return transformation_vels, transformation_accs
...
def main_a(cfg):
    ...
    ### TODO: it should be adjusted, including what you've set during the diffusion evaluation; --> to 0.05
    # fixed_y_len = 0.1
    ...
    #### get manipulator infos #### #### get manipulator infos #### --> st_len_one_side ###
    ...
    ''' Define the manipulator transformation process '''  ## get manipulator infos ## ## ## ##
    ...
    if len(prev_saved_best_ckpt_fn) == 0:
        prev_saved_best_ckpt_fn = None ## tot len one side ##
    ...
    ##### TODO: load the saved information with batched link rotations and translations; calculate the rotation and translation accs; initialize the uni_manip's rotation and translation accs using the pre-loaded values #####
    ##### Load information from saved dict info nf #####
    saved_dict_info_fn = "/data/xueyi/uni_manip/exp/eval_v2_target_data_savemore_/eval_v2_target_data_savemore__use_t_1000_/sampled_pcd_wact_dict_projsample_iter_0.npy"
    saved_dict_info = np.load(saved_dict_info_fn, allow_pickle=True).item()
    link_rotations = saved_dict_info['link_rotations']
    link_translations = saved_dict_info['link_translations']
    selected_bsz_idx = 0
    
    ### 
    sampled_link_rotations = link_rotations[selected_bsz_idx]
    sampled_link_translations = link_translations[selected_bsz_idx] ## get the selected link rotatins and the translations ##
    # nn_timesteps, nn_links, link_translational_dims #
    # sampled_link_translations
    
    print(f"sampled_link_rotations: {sampled_link_rotations.shape}, sampled_link_translations: {sampled_link_translations.shape}")
    
    # transformation_vels, transformation_accs
    link_rotation_vels, link_rotation_accs = get_rotational_translations_vels_accs(sampled_link_rotations)
    link_translation_vels, link_translation_accs = get_rotational_translations_vels_accs(sampled_link_translations)
    
    # if link_rotation_accs.shape[-1] == 1:
    #     link_rotation_accs = link_rotation_accs[..., 0] ## ## get the link rotations and the translations ## ##
    ## get the translational accs ##
    uni_manip_2d.load_optimized_link_accs(link_rotation_accs, link_translation_accs)
    
    ## TODO: set the link rotational and translational accs ##
...
def main():
    ...
    cfg.run.exp_folder_tag = f"eval_expv4_projected_task_{cfg.task.task_idx}"
