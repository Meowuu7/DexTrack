...
## TODO: implement the reduced coordiante systems ##
## TODO: implement the force/torque driven articulated systems ##

## TODO: find a good optimization strategy ##
## 
...
class UniManip2D:
    def __init__(self, nn_particles, nn_links, nn_joints, nn_timesteps, dim, dt, obj_info_fn, link_joint_pos, link_joint_dir, link_parent_idx, exp_tag=None, cur_transformation_penalty_term=1000, cfg=None):
        ...
        ## not for the link SE(3) states but the per-point accs ## 
        
        ## primal is the particles ## 
        ## reduced is the joint and links ## 
        ## get the rotational translational and rotational accs ##
        ##### ==== Root link rotational and translational accs ==== #####
        if self.dim == 2:
            self.root_link_rotational_accs = ti.Vector.field(1, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
            self.root_link_rotational_vels = ti.Vector.field(1, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
            self.root_link_rotational_vecs = ti.Vector.field(1, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
        else:
            self.root_link_rotational_accs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
            self.root_link_rotational_vels = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
            self.root_link_rotational_vecs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
        
        if self.dim == 2:
            self.child_link_rotational_accs = ti.Vector.field(1, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) ## childelin kangle accs ##
            self.child_link_rotational_vels = ti.Vector.field(1, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) ## childelin kangle accs ##
            self.child_link_rotational_vecs = ti.Vector.field(1, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) ## childelin kangle accs ##
            ## get the child link rotational vecs ##
        else:
            self.child_link_rotational_accs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) ## childelin kangle accs ## ## link rotational accs for the child link
            self.child_link_rotational_vels = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) ## childelin kangle accs ##
            self.child_link_rotational_vecs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) ## childelin kangle accs ##
        
        self.root_link_translational_accs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
        self.root_link_translational_vels = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
        self.root_link_translational_vecs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, ), needs_grad=True)
        
        self.child_link_translational_accs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) 
        self.child_link_translational_vels = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) 
        self.child_link_translational_vecs = ti.Vector.field(self.dim, dtype=ti.f32, shape=(self.nn_timesteps, self.nn_links), needs_grad=True) 
        
        ## use the forward 
        ## rot
        ### TODO：from link root and child joint rotations and translations to the particle accs, vels, and vecs ###
        ## 
        ## the link rotation angle accs ##
        ## the link translational vec accs ## 
        ## particles xs and the particles vels ##
        ## as the initial states values for the hard constraints simulations ##
        ## x_t = R_t(x - x_0) + t_t
        ## x_{t - 1} = R_{t - 1} (x - x_0) + t_{t - 1} ## get the xs and Rs and the ts ## 
        ## x_{t - 1} + dt * (v_{t - 1} + dt * a_t) = R_{t} (x_0 - x_0^c) + t_{t}
        ## (R_t - R_{t - 1} + R_{t - 1}) (x_0 - x_0^c) + t_t - t_{t - 1} + t_{t - 1} = 
        ## (R_t - R_{t - 1} + rotations #
        ## but the particles movements ##
        
        
        ''' Link SE(3) States '''
        ##### ==== link rotational vecs ==== #####
