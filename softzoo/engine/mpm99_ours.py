...
# joint_pos = ti.Vector([0.4, 0.37], dt=ti.f32)
...
def substep():
    ...
    for p in x:  # Particle state update and scatter to grid (P2G)
        ...
        if material[p] == 0:  # liquid
            mu = 0.0
        U, sig, V = ti.svd(F[p])
        # Avoid zero eigenvalues because of numerical errors # numerical errors # # numerical errors # # numerical errors #
    ...
    for p in x:  # grid to particle (G2P)
        ...
        for i, j in ti.static(ti.ndrange(3, 3)): # 
            # loop over 3x3 grid node neighborhood # loop over
        ...
        v[p], C[p] = new_v, new_C # 
        x[p] += dt * v[p]  # advection # advection # # advection # # advection # # advection #
...
def main():
    ...
    while not gui.get_event(ti.GUI.ESCAPE, ti.GUI.EXIT):
        ...
        ## transferred to the gird space ## ## grid space ## ## grid space ##

## from
# # fro mthe grid velocities to the control information in the reduced dimensional systems ### ? # How to control that in them? # how to control ## control a soft robot # in the control a soft robot # soft robot; soft robot # 
#  
