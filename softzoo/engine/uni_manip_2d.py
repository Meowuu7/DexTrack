...
@ti.data_oriented
class UniManip2D:
    def __init__(self, nn_particles, nn_links, nn_joints, nn_timesteps, dim, dt, obj_info_fn, link_joint_pos, link_joint_dir, link_parent_idx, exp_tag=None):
        ...
        # self.link_idx_to_pcs, to_parent, to_joint_info #
        # get link idx to pcs; 
    ...
    def get_goal_reaching_loss_backward(self, s: ti.i32):
        for p in range(self.nn_particles):
            ...
            if ti.abs(cur_particle_goal_diff - self.goal_reaching_loss[None]) < 1e-3:
                self.particle_xs.grad[s, p] += 2 * (cur_particle_pos - goal_pos) * self.goal_reaching_loss.grad[None]
            # self.goal_reaching_loss[None] = ti.min(cur_particle_goal_diff, self.goal_reaching_loss[None])
    
    
    @ti.kernel
    def get_total_loss(self):
        self.tot_loss[None] += self.transformation_penalty_loss[None] * 1000 + self.transformation_acc_reg_loss[None] * 0.01 + self.goal_reaching_loss[None]
    ...
    def optimize_iter(self, ):
        ...
        # print(f"After tot_loss grad: goal_reaching_loss_grad = {goal_reaching_loss_grad}, transformation_penalty_loss_grad: {self.transformation_penalty_loss.grad[None]}")
    ...
    # best_ckpt_sv_fn = optimize(nn_tot_iters=40000, n_terminate=False)
...
if __name__=='__main__':
    ...
    # test_link_gen() # test link gen 
    # exit(0)
    
    # # python softzoo/engine/uni_manip_2d.py
    
    # UniManip2D:
    # def __init__(self, nn_particles, nn_links, nn_joints, nn_timesteps, dim, dt, obj_info_fn, link_idx_to_parent, link_idx_to_joint_info, joint_idx_to_constraint_factor):
    ...
    ''' Start optimization '''
    for i_inst in range(len(tot_len_one_side)):
    ...
    ## the uni_manip_2d ## ## formulation ##
    ## constraints formulation ##
    ## constraints optimization ##
    ## constraints optimization ##
    ## a more complex task #
    ## a completed transformation process ## ## a completed and parameterized transformation process ##
    ## a ## 
    
    ## the relations between the manipulator in the two stages ##
    # acc interpolations -- project to the constrained manifold #
