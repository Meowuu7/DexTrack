...
export hist_cond_partial_hand_info=""
export hist_cond_partial_obj_info=""
export hist_cond_partial_obj_pos_info=""
########### Default settings ###########
...
export resume_checkpoint='/cephfs/xueyi/uni_manip/tds_diffusion_exp/allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_ws60_centralize_partialhandinfo_/model000250001.pt'
####### AE Diffusion Settings -- train_Diff #######
...
export resume_checkpoint='/cephfs/xueyi/uni_manip/tds_diffusion_exp/allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_Diff_ws60_centralize_handobjinfo_/model000900002.pt'
export resume_checkpoint='/cephfs/xueyi/uni_manip/tds_diffusion_exp/allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_Diff_ws60_centralize_handobjinfo_/model002980001.pt'
####### AE Diffusion Settings -- train_Diff with task cond #######
...
####### Add the history partial infos ########
# export hist_cond_partial_hand_info="--hist_cond_partial_hand_info"
export hist_cond_partial_hand_info=""
export hist_cond_partial_obj_info=""
export hist_cond_partial_obj_pos_info=""
####### Add the history partial infos ########
...
export exp_tag=allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_Diff_ws60_centralize_partialhandinfo_
# can predict objrv
export exp_tag=allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_Diff_ws60_centralize_handobjinfo_
export exp_tag=allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_Diff_task_cond_ws60_centralize_partialhandinfo_
export exp_tag=allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_Diff_task_cond_ws60_centralize_partialhandinfo_v2_
export exp_tag=allegro_tracking_kine_diff_AE_Diff_traj_forcasting_AEDiff_AE_Diff_task_cond_ws60_centralize_handobjinfo_
# export exp_tag=allegro_tracking_traj_forcasting_ws60_centralize_handobjinfo_
...
# bash get the main traj forcasting diff # strongly conditioned things # masked rewards for hand, object or the hand keypoints #
# #
# bash scripts/K2_run_main_3d_pc_traj_forcasting_diff.sh


CUDA_VISIBLE_DEVICES=${cuda_ids} python main_3d_pc.py  ${task_cond}  --resume_checkpoint=${resume_checkpoint} --exp_tag=${exp_tag} --save_interval=${save_interval}  ${debug} --statistics_info_fn=${statistics_info_fn} --training_setting=${training_setting} ${single_inst} --batch_size=${batch_size} ${training_use_jointspace_seq} ${diff_task_space} ${diff_task_translations} ${kine_diff} ${tracking_ctl_diff} ${sampling} --use_t=${use_t} --target_grab_inst_tag=${target_grab_inst_tag} --target_grab_inst_opt_fn=${target_grab_inst_opt_fn} ${AE_Diff} ${train_AE} ${train_Diff} ${cond_diff_allparams} ${multi_inst} --slicing_ws=${slicing_ws} ${slicing_data} --sim_platform=${sim_platform} --grab_inst_tag_to_optimized_res_fn=${grab_inst_tag_to_optimized_res_fn} --grab_inst_tag_to_opt_stat_fn=${grab_inst_tag_to_opt_stat_fn} --task_cond_type=${task_cond_type} --history_ws=${history_ws} --taco_inst_tag_to_optimized_res_fn=${taco_inst_tag_to_optimized_res_fn} --taco_interped_fr_grab_tag=${taco_interped_fr_grab_tag} --taco_interped_data_sv_additional_tag=${taco_interped_data_sv_additional_tag} --exp_additional_tag=${exp_additional_tag}  --sub_task_cond_type=${sub_task_cond_type} --task_inherit_info_fn=${task_inherit_info_fn} ${glb_rot_use_quat} ${use_kine_obj_pos_canonicalization} --data_statistics_info_fn=${data_statistics_info_fn} --kine_diff_version=${kine_diff_version} ${use_taco_data} --num_frames=${num_frames} --maxx_inst_nn=${maxx_inst_nn} ${w_glb_traj_feat_cond} ${canonicalize_features} --obj_type_to_kinematics_traj_dict_fn=${obj_type_to_kinematics_traj_dict_fn} ${canonicalize_obj_pts} --forcasting_window_size=${forcasting_window_size} ${w_history_window_index} ${partial_hand_info} ${partial_obj_info} ${diff_contact_sequence} ${w_masked_future_cond} ${st_ed_state_cond} ${partial_obj_pos_info} ${centralize_info} --glb_feat_per_skip=${glb_feat_per_skip} --history_window_size=${history_window_size} ${scale_clip_data} ${hist_cond_partial_hand_info} ${hist_cond_partial_obj_info} ${hist_cond_partial_obj_pos_info}
