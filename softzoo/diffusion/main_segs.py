...
from training_loop_segs import TrainLoop
...
from dataset.get_data import get_dataset_loader_segs
from model_util import create_model_and_diffusion_segs
...
def main():
    ...
    # exp_tag # exp_tag # exp_tag #
    cfg.exp_tag = f"{cfg.exp_tag}_segs_"
    args = cfg
    ...
    data = get_dataset_loader_segs(name=args.dataset.dataset_type, batch_size=args.training.batch_size, num_frames=args.dataset.num_frames, args=args)

    print("creating model and diffusion...")
    
    # model, diffusion = create_model_and_diffusion(args, data)  ## get the
    model, diffusion = create_model_and_diffusion_segs(args)
    model.cuda()
...
# CUDA_VISIBLE_DEVICES=0 python main_segs.py  # the guided acton sequences ## --- sampel for the action sequences ## ## act sequences ##
# CUDA_VISIBLE_DEVICES=1 python main_segs.py  # the action seuqneces and the guided action sequences? #
# 
# 
