...
class Transformer_Net_PC_Seq_V3_KineDiff_AE_V4(nn.Module):
    ...
    def __init__(self, n_layers: int, input_dims: dict, hidden_mlp_dims: dict,
                 output_dims: dict, act_fn_in: nn.ReLU(), act_fn_out: nn.ReLU(), traj_cond=False, w_timestep_cond=False, w_history_window_index=False, w_glb_traj_cond=True, st_ed_state_cond=False):
        super().__init__()
        ...
        self.pos_hidden_dim = hidden_mlp_dims['X']
        self.feat_hidden_dim = hidden_mlp_dims['feat']
        self.st_ed_state_cond = st_ed_state_cond
        # self.concat_two_dims = input_dims['concat_two_dims']
        ...
        self.w_history_window_index = w_history_window_index
        self.w_glb_traj_cond = w_glb_traj_cond
        ...
        # transformer model v4 #
        
        if self.traj_cond:
            # point cloud, the conditional 
            self.cond_feat_in_dim = input_dims.get('cond_feat', self.feat_in_dim)
            # self.history_cond_feat_in_dim = self.cond_feat_in_dim #  self.feat_in_dim
            self.history_cond_feat_in_dim = input_dims.get('hist_cond_feat', self.cond_feat_in_dim)
            self.cond_input_process_pc = InputProcessObjBasePC(3, self.latent_dim, layernorm=True, without_glb=False, only_with_glb=False, zero_init=True)
            self.cond_input_process_feat = InputProcessObjBaseV5( self.cond_feat_in_dim, self.feat_hidden_dim, layernorm=True, without_glb=False, only_with_glb=False, zero_init=True)
            ...
            if self.st_ed_state_cond:
                pts_in_dim = 3
                pose_in_dim = 22 + 3 + 4
                self.cond_input_process_feat = InputProcessObjBasePCGoalState(pts_in_dim, pose_in_dim, self.latent_dim, layernorm=True, without_glb=False, only_with_glb=False, zero_init=True)
            
            
            self.cond_input_process_feat_hist = InputProcessObjBaseV5( self.history_cond_feat_in_dim, self.feat_hidden_dim, layernorm=True, without_glb=False, only_with_glb=False, zero_init=True)
            self.cond_input_positional_encoder_hist = PositionalEncoding(self.latent_dim, self.dropout)
            
            if self.w_history_window_index:
                self.cond_input_position_indicating_encoder_hist = PositionalEncoding(self.latent_dim, self.dropout, max_len=self.maxx_timestep_cond)
    ...
    def forward(self, pts_feat, feat_feat, y, node_mask=None, cond=None):
        ...
        if self.traj_cond: # # # traj cond # # # # # # traj cond # # 
            cond_X = cond['X']
            cond_E = cond['E']
            ...
            if self.st_ed_state_cond:
                # print(f"Using st_ed state conditions")
                st_ed_feats = torch.stack( # bsz x 2 x feat_dim
                    [ cond_E[:, 0, :], cond_E[:, -1, :] ], dim=1
                )
                if len(cond_X.size()) == 4:
                    cond_X = cond_X[:, :, 0]
                cond_encoded_feat = self.cond_input_process_feat(cond_X, st_ed_feats)
                # cond glb feat #
                cat_cond_feat_obj_embedding = torch.cat(
                    [ cond_glb_feat,  cond_encoded_feat ], dim=-1
                )
                cat_cond_feat_obj_embedding = cat_cond_feat_obj_embedding.unsqueeze(0)
            else:
                ...
                cond_encoded_feat = self.cond_input_positional_encoder(cond_encoded_feat)
                cond_encoded_feat = self.cond_input_transformer_encoder(cond_encoded_feat)
                cond_encoded_feat = cond_encoded_feat[0:1]
                
                expanded_cond_pts_feat = cond_glb_feat.unsqueeze(0).repeat(cond_encoded_feat.size(0), 1, 1)
                
                # cat feat obj embedding #
                cat_cond_feat_obj_embedding = torch.cat(
                    [expanded_cond_pts_feat, cond_encoded_feat], dim=-1
                )
            
            if self.w_glb_traj_cond:
                cat_feat_obj_embedding = cat_feat_obj_embedding + cat_cond_feat_obj_embedding
            else:
                pass
            ...
            # 
            if self.w_history_window_index:
                # print(f"Using history window indexes")
                cond_E_window_idxes = cond['history_E_window_idx']
                cond_E_window_idxes = cond_E_window_idxes.long()
                cond_encoded_feat_hist = self.cond_input_position_indicating_encoder_hist.forward_batch_selection(cond_encoded_feat_hist, cond_E_window_idxes)
...
class Transformer_Net_PC_Seq_V3_KineDiff_AE_V6(nn.Module):
    def __init__(self, n_layers: int, input_dims: dict, hidden_mlp_dims: dict,
                 output_dims: dict, act_fn_in: nn.ReLU(), act_fn_out: nn.ReLU(), traj_cond=False, w_glb_traj_feat_cond=False, w_timestep_cond=False, use_clip_glb_features=False, clip_feat_dim=512):
        super().__init__()
        ...
        self.use_clip_glb_features = use_clip_glb_features
        ...
        self.w_hand = input_dims.get('w_hand', True)
        self.w_obj_pos = input_dims.get('w_obj_pos', True)
        self.w_obj_ornt = input_dims.get('w_obj_ornt', True)
        
        self.input_process = InputProcessObjBaseV7(self.feat_in_dim, self.latent_dim, w_hand=self.w_hand, w_obj_pos=self.w_obj_pos, w_obj_ornt=self.w_obj_ornt)
        ...
        if self.w_glb_traj_feat_cond:
            ...
            if self.use_clip_glb_features:
                tot_feat_in_dim = self.latent_dim + self.clip_feat_dim
        else:
            tot_feat_in_dim = self.latent_dim
    ...
    def forward(self, pts_feat, feat_feat, tot_feat_feat=None, tot_obj_pts=None, y=None, node_mask=None, cond=None, history_window_index=None):
        ...
        # pts_feat -- bsz x nn_ts x nn_pts x 3 # pts feat #
        # feat_feat -- bsz x nn_ts x (nn_hand_pose_dim + nn_obj_pos_dim + nn_obj_ornt_dim) #
        if self.w_hand and self.w_obj_pos and self.w_obj_ornt:
            x_hand_pose, x_obj_pos, x_obj_ornt = feat_feat[..., : self.feat_in_dim], feat_feat[..., self.feat_in_dim: self.feat_in_dim + 3], feat_feat[..., self.feat_in_dim + 3: self.feat_in_dim + 7]
        elif self.w_obj_pos:
            x_obj_pos = feat_feat
            x_hand_pose = None
        ...
        #
        encoded_feat = self.input_transformer_encoder(encoded_feat) # nn_ts x nn_bsz x latent_dim #
        # input transformer encoder #
        last_encoded_feat = encoded_feat[-1:, :, :] # get the last encoded #
        ## NOTE: assume the input conditional window is with the same length as the output windo
        # print(f"last_encoded_feat: {last_encoded_feat.size()}, encoded_feat: {encoded_feat.size()}")
        ...
        if self.w_glb_traj_feat_cond:
            ...
            if self.use_clip_glb_features:
                last_glb_encoded_feat = tot_feat_feat # bsz x nn_features
                expanded_glb_encoded_feat = last_glb_encoded_feat.unsqueeze(0).contiguous().repeat(encoded_feat.size(0), 1, 1).contiguous()
                expanded_encoded_feat = torch.cat([ expanded_encoded_feat, expanded_glb_encoded_feat ], dim=-1) 
            else:
                glb_hand_pose, glb_obj_pos, glb_obj_ornt = tot_feat_feat[..., : self.feat_in_dim], tot_feat_feat[..., self.feat_in_dim: self.feat_in_dim + 3], tot_feat_feat[..., self.feat_in_dim + 3: self.feat_in_dim + 7]
                glb_encoded_feat = self.input_process_glb_traj(glb_hand_pose, glb_obj_pos, glb_obj_ornt, tot_obj_pts)
                glb_encoded_feat = glb_encoded_feat.contiguous().transpose(1, 0).contiguous() 
...
class InputProcessObjBaseV7(nn.Module):
    def __init__(self, input_feats, latent_dim, w_hand=True, w_obj_pos=True, w_obj_ornt=True): 
        super().__init__()
        ...
        if w_obj_ornt:
            nn_encoding_parts += 1
        nn_final_latent_dim = self.latent_dim * (nn_encoding_parts + 1)
    ...
    def forward(self, x_hand_pos=None, x_obj_pos=None, x_obj_ornt=None, x_pts=None):
        tot_feats_list = []
        if x_hand_pos is not None:
            x_hand_feat = self.feats_encoding_net(x_hand_pos)
            tot_feats_list.append(x_hand_feat)
        if x_obj_pos is not None:
            x_obj_pos_feat = self.obj_pos_encoding_net(x_obj_pos)
            tot_feats_list.append(x_obj_pos_feat)
        if  x_obj_ornt is not None:
            x_obj_ornt_feat = self.obj_ornt_encoding_net(x_obj_ornt)
            tot_feats_list.append(x_obj_ornt_feat)
        # x_obj_ornt_feat = self.obj_ornt_encoding_net(x_obj_ornt)
        ...
        tot_feats_list.append(x_glb_pts_feat)
        
        x_tot_feat = torch.cat(tot_feats_list, dim=-1)
...
class InputProcessObjBasePCGoalState(nn.Module):
    def __init__(self, input_feats, input_feat_dim, latent_dim, layernorm=True, without_glb=False, only_with_glb=False, zero_init=False): 
        super().__init__()
        # self.data_rep = data_rep #
        self.input_feats = input_feats # 21 * 3 + 3 + 3 --> for each joint + 3 pos + 3 normals #
        self.latent_dim = latent_dim
        ...
        self.st_frame_pose_encoding_net = nn.Sequential(
            nn.Linear(self.input_feat_dim, self.latent_dim), nn.ReLU(),
            nn.Linear(self.latent_dim, self.latent_dim) # latent dim of the st frame pose
        )
        
        
        self.ed_frame_pose_encoding_net = nn.Sequential(
            nn.Linear(self.input_feat_dim, self.latent_dim), nn.ReLU(),
            nn.Linear(self.latent_dim, self.latent_dim) # latent dim of the st frame pose
        )
        
        self.final_feat_transformation_net = nn.Sequential(
    ...
    def init_weights(self, ):
        for i_module, module in enumerate(self.st_frame_pose_encoding_net):
            if isinstance(module, nn.Linear):
                torch.nn.init.zeros_(module.bias)
                torch.nn.init.xavier_uniform_(module.weight)
        for i_module, module in enumerate(self.ed_frame_pose_encoding_net):
            if isinstance(module, nn.Linear):
                torch.nn.init.zeros_(module.bias)
                torch.nn.init.xavier_uniform_(module.weight)
        for i_module, module in enumerate(self.final_feat_transformation_net):
            if isinstance(module, nn.Linear):
                torch.nn.init.zeros_(module.bias)
                torch.nn.init.xavier_uniform_(module.weight)
        for i_module, module in enumerate(self.pts_feats_encoding_net):
    ...
    def forward(self, x, st_ed_feat, rt_glb=False): # decode relative positions and others ## # cond on the points for points output #
        # bs, nframes, njoints, nfeats = x.shape #
        # bsz x nf x nnj x (3 + nnb x (3 + 3))  # bsz x nf x nnb x (latent_dim)
        # x: bsz x nf x nnb x (3 + 3 + 21 * 3) # x.size()
        # bsz, nf, nnb = x.size()[:3]
        ...
        x_glb_emb, _ = torch.max(x_pts_emb, dim=1, keepdim=False)
        x_glb_emb = self.glb_feats_encoding_net(x_glb_emb)
        
        # x_pts_emb =
        
        st_feat = st_ed_feat[:, 0, :] # bsz x nn_input_dim
        ed_feat = st_ed_feat[:, 1, :] # bsz x nn_input_dim 
        encoded_st_feat = self.st_frame_pose_encoding_net(st_feat)
        encoded_ed_feat = self.ed_frame_pose_encoding_net(ed_feat)
        joint_task_cond_feat = torch.cat(
            [ x_glb_emb, encoded_st_feat, encoded_ed_feat ], dim=-1 # getthe glb jembedding
        )
        joint_task_cond_feat = self.final_feat_transformation_net(joint_task_cond_feat)
        
        return joint_task_cond_feat
