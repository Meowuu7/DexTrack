...
class TrainLoop:
    ...
    def predict_single_step(self, batch, use_t=None):
        # self.mp_trainer.zero_grad()
        # use_t is not None # use_t is not None #
        # tot_samples = []
        ...
        for i in range(0, nn_bsz, self.microbatch):
            ...
            ## sample ## predict single step ##
            ## sample from the model --- the target sample should be in the sahpe of micro['X'].shape ### shape ##
            # shape = micro['X'].shape # micro X #
            ...
            ## training loop segs #### training loop segs ##
            ## primal 
            ## primal space actions distribution; actions jdistribtuion; 
