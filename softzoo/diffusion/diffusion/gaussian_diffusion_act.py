...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        node_masks = th.ones((x.shape[0], x.shape[1]), device=x.device) #### construct node masks
            
        
        # model_output = model(x, self._scale_timesteps(t), **model_kwargs)
        model_output = model(x, batched_scaled_t, node_mask=node_masks)
        ...
        #     )
        # print(f"model_mean: {model_mean.shape}, model_log_variance: {model_log_variance.shape}, pred_xstart: {pred_xstart.shape}, x: {x.shape}")
        assert (
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # print(f"batched_scaled_t: {batched_scaled_t.size()}")
            
            node_masks = th.ones((x_t.shape[0], x_t.shape[1]), device=x_t.device)
            
            # print(f'x_start: {x_start.size()}, x_t: {x_t.size()}')
            # print(f"model: {model}")
            model_output = model(x_t, batched_scaled_t, node_mask=node_masks)
            
            # print(f"model_output: {model_output.size()}, x_t: {x_t.size()}, x_start: {x_start.size()}")
            ...
            #         )[0],
            #         ModelMeanType.START_X: cur_x_start,
            #         ModelMeanType.EPSILON: cur_noise,
